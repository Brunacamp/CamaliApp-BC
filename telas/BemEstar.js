...
// Modificação no componente SmallCard para aceitar e usar a prop onPress
const SmallCard = ({ title, imageSource, onPress }) => ( 
    <TouchableOpacity onPress={onPress} style={[cardPequeno.card, { backgroundColor: '#FFF9EA' }]}>
        <View style={cardPequeno.header}>
            <Text style={cardPequeno.title}>{title === "Dados disponíveis" ? "Chat Orientadora" : "Cuidar da Mente"}</Text>
            <MaterialCommunityIcons name="chevron-right" size={24} color="#444" />
        </View>
        
        <View style={cardPequeno.content}>
            {imageSource && (
                <Image source={imageSource} style={cardPequeno.imagemFundo} resizeMode="cover" />
            )}
            <Text style={cardPequeno.subtitulo}>{title}</Text>
        </View>
    </TouchableOpacity>
);
...
const BemEstarMentalScreen = () => {
    ...
    const handleEstadoEmocionalPress = () => {
        navigation.navigate('estadoEmocional'); 
    };
            ...
            >
                ...
                <ScrollView contentContainerStyle={styles.container}>
                    ...
                    <View style={styles.cardPequenoContainer}>
                        {/* SmallCard para Chat Orientadora com navegação */}
                        <SmallCard 
                            imageSource={require('../assets/imgChat.png')}
                            onPress={handleChatOrientadoraPress}
                        />
                        {/* SmallCard para Cuidar da Mente com navegação */}
                        <SmallCard 
...
const styles = StyleSheet.create({
    ...
    cardHeader: { 
        ...
        alignItems: 'center',
        marginBottom: 15,
        // Adicionado para alinhar o conteúdo do cabeçalho à esquerda, como na imagem
    ...
    cardTitulo1: {
        ...
        fontWeight: 'bold',
        color: '#161C4E',
        // Removendo margens negativas e fixas que estavam causando o desalinhamento
        // O alinhamento será corrigido pelo `cardHeader` e `emocaoIcon`
    },
    emocaoIcon: {
...
const emocoesestilo = StyleSheet.create({
    emojiStripContainer: {
        ...
        overflow: 'hidden',
        // Adicionado para centralizar o container do emoji strip dentro do card
        alignSelf: 'center',
